...
for i, task_classes in enumerate(tasks):
    ...
    # --- Evaluation for Forgetting (task-restricted accuracy) ---
    model.eval()
    with torch.no_grad():
        for prev_idx in range(i + 1):
            ...
            for batch in test_loader:
                ...
                if mask.sum() == 0:
                    continue
                out = model(batch.x, batch.edge_index)
                # Task-restricted accuracy (fairer): select only logits for this task
                task_out = out[mask][:, local_prev]
                task_pred = task_out.argmax(dim=-1) + prev_task_classes[0]
                correct += (task_pred == batch.y[mask]).sum().item()
                total += mask.sum().item()
