"""
GraphEWC — Topology-Weighted Elastic Weight Consolidation
==========================================================
Extends standard EWC with graph topology weighting:
    F_i_graph = F_i_standard * (1 + α * mean_degree_of_batch_nodes)

The intuition: parameters important to high-degree nodes matter more,
because those nodes influence more neighbours through message passing.

α controls topology weight strength.
Sweep α: [0.1, 0.5, 1.0, 2.0]
Uses best standard EWC λ as base (1e5 — best stability-plasticity balance)
Saves to ./res/graph_ewc/
"""
...
EWC_LAMBDA = 1e5   # base EWC strength (best from standard EWC sweep)
RES_DIR = PROJECT_ROOT / "res" / "graph_ewc"
RES_DIR.mkdir(parents=True, exist_ok=True)


# ─── Topology-Weighted Fisher ───────────────────────────────────────────────
...
def compute_graph_fisher(model, data, class_weights, alpha, device, n_samples=2000):
    """Compute topology-weighted Fisher Information Matrix.

    F_i_graph = F_i_standard * (1 + α * mean_degree_of_training_nodes)

    The topology weight scales ALL Fisher entries uniformly by the mean
    degree of R1 training nodes used for computation. This makes the
    EWC penalty stronger for graphs with higher connectivity.
    """
    model.eval()
    data = data.to(device)
    ...
    if n_samples < len(r1_indices):
        perm = torch.randperm(len(r1_indices), device=device)[:n_samples]
        r1_indices = r1_indices[perm]

    # Compute mean degree of sampled nodes
    degrees = compute_node_degrees(data).to(device)
    mean_degree = degrees[r1_indices].mean().item()
    topo_weight = 1.0 + alpha * mean_degree
    ...
    # Average and apply topology weight
    n = len(r1_indices)
    for n_key in fisher:
        fisher[n_key] = (fisher[n_key] / n) * topo_weight

    return fisher, mean_degree, topo_weight


# ─── GraphEWC Training Loop ─────────────────────────────────────────────────
...
def train_epoch_graph_ewc(model, data, optimizer, task_labels, class_weights,
...
def run_graph_ewc_experiment(alpha, data, class_weights, device, seed):
    ...
    # Compute topology-weighted Fisher
    print(f"        Computing graph-weighted Fisher (α={alpha})...")
    fisher, mean_deg, topo_w = compute_graph_fisher(
        model, data, class_weights, alpha, device)
    print(f"        Mean degree: {mean_deg:.1f}, topology weight: {topo_w:.2f}")

    # Round 2 with GraphEWC
    for _ in range(EPOCHS_PER_ROUND):
        loss, ewc_loss = train_epoch_graph_ewc(
            model, data, optimizer, JS_LABELS, class_weights,
            fisher, optimal_params, EWC_LAMBDA, device)
    ...
    return {
        ...
        "alpha": alpha, "mean_degree": mean_deg, "topo_weight": topo_w,
    }
...
def save_all(all_results, summary):
    ...
    json_data = {
        "method": "graph_ewc",
        "config": {"alpha_values": ALPHA_VALUES, "ewc_lambda": EWC_LAMBDA,
                   "seeds": SEEDS, "epochs_per_round": EPOCHS_PER_ROUND},
        "results_by_alpha": {str(k): v for k, v in all_results.items()},
    ...
    _plot_alpha_sweep(summary); print(f"  ✓ alpha_sweep.png")
    _save_text_summary(summary); print(f"  ✓ forgetting_summary.txt")
    print(f"\nAll results saved to {RES_DIR}/")


def _best_alpha(summary):
    # Best = highest sum of normalized recovery + plasticity (Pareto balance)
    def score(k):
        return summary[k]["recovery_ratio"]["mean"] + summary[k]["plasticity"]["mean"]
    return max(summary.keys(), key=score)


def _plot_training_curves(all_results, summary):
    best = _best_alpha(summary); runs = all_results[best]
    ...
    plt.suptitle(f"GraphEWC (α={best}, λ={EWC_LAMBDA:.0e})", fontsize=14, fontweight="bold", y=1.02)
    plt.tight_layout()
...
def _plot_accuracy_comparison(all_results, summary):
    ...
    ax.bar(x_pos + width/2, r2_m, width, yerr=r2_s, capsize=3,
           label="After Round 2 (GraphEWC)", color="#e74c3c", edgecolor="white")
    ax.axvline(x=2.5, color="gray", linestyle="--", alpha=0.5)
    fgt = summary[best]["forgetting"]
    ax.set_title(f"GraphEWC (α={best})\nForgetting = {fgt['mean']:.4f} ± {fgt['std']:.4f}",
...
def _plot_forgetting_viz(all_results, summary):
    ...
    ax.text(E + E / 2, 1.08, "Round 2\n(JS + GraphEWC)", ha="center", fontsize=11)
    ax.set_xlabel("Epoch", fontsize=12); ax.set_ylabel("Test Accuracy", fontsize=12)
    ax.set_title(f"GraphEWC (α={best}) — Forgetting", fontsize=14, fontweight="bold")
...
def _plot_alpha_sweep(summary):
    fig, ax1 = plt.subplots(figsize=(10, 6))
    alphas = sorted(summary.keys())
    recovery = [summary[a]["recovery_ratio"]["mean"] for a in alphas]
    recovery_std = [summary[a]["recovery_ratio"]["std"] for a in alphas]
    js_acc = [summary[a]["r2_js_acc"]["mean"] for a in alphas]
    js_std = [summary[a]["r2_js_acc"]["std"] for a in alphas]
    ax1.errorbar(alphas, recovery, yerr=recovery_std, marker="o", color="#2ecc71",
                 linewidth=2, capsize=5, label="Recovery Ratio")
    ax1.set_xlabel("α (topology weight)", fontsize=12)
    ax1.set_ylabel("Recovery Ratio", fontsize=12, color="#2ecc71")
    ax1.tick_params(axis="y", labelcolor="#2ecc71")
    ax2 = ax1.twinx()
    ax2.errorbar(alphas, js_acc, yerr=js_std, marker="s", color="#3498db",
    ...
    ax1.set_title(f"GraphEWC — α Sweep (λ={EWC_LAMBDA:.0e})", fontsize=14, fontweight="bold")
    ax1.grid(True, alpha=0.3); plt.tight_layout()
    fig.savefig(RES_DIR / "alpha_sweep.png", dpi=150, bbox_inches="tight"); plt.close()


def _save_text_summary(summary):
    lines = ["=" * 65, f"GRAPH EWC (λ={EWC_LAMBDA:.0e}, topology-weighted Fisher)", "=" * 65, ""]
    for alpha in sorted(summary.keys()):
        ...
        lines.append("")
    best = _best_alpha(summary)
    lines.append(f"Best α: {best} (highest recovery+plasticity sum)")
...
def main():
    ...
    print(f"Method: GraphEWC (topology-weighted)")
    print(f"α values: {ALPHA_VALUES}")
    print(f"Base λ: {EWC_LAMBDA:.0e}")
    print(f"Seeds: {SEEDS}\n")

    all_results = {a: [] for a in ALPHA_VALUES}
    for i, seed in enumerate(SEEDS):
        print(f"\n{'='*60}\nSEED {seed} ({i+1}/{len(SEEDS)})\n{'='*60}")
        data, class_weights, info = build_unified_graph(seed)
        for alpha in ALPHA_VALUES:
            print(f"\n  α = {alpha}")
            result = run_graph_ewc_experiment(alpha, data, class_weights, device, seed)
