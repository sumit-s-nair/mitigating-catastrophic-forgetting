"""
Experience Replay — Herding Buffer Selection
=============================================
Per class: compute TF-IDF centroid of training nodes, select N/3 nodes
closest to that centroid. This is deterministic given the data split.

Herding selects the most "representative" examples, which should yield
better memory efficiency than random sampling.

Sweep buffer sizes: [100, 500, 1000, 2000]
Saves to ./res/replay_herding/
"""
...
RES_DIR = PROJECT_ROOT / "res" / "replay_herding"
RES_DIR.mkdir(parents=True, exist_ok=True)


# ─── Buffer Construction ─────────────────────────────────────────────────────

def build_herding_buffer(data, buffer_size):
    """Select nodes closest to each class centroid in TF-IDF feature space.

    Deterministic: no randomness, depends only on the data split (train_mask).
    Per class, compute centroid of training features, rank by L2 distance,
    take top-N/3 closest nodes. Returns CPU tensor.
    """
    per_class = buffer_size // len(PYTHON_LABELS)
    selected = []

    for label_id in PYTHON_LABELS:
        class_mask = data.train_mask & (data.y == label_id)
        class_indices = class_mask.nonzero(as_tuple=True)[0]
        class_features = data.x[class_indices]  # (n_class, 512)

        # Compute centroid
        centroid = class_features.mean(dim=0, keepdim=True)  # (1, 512)

        # L2 distance from each node to centroid
        dists = torch.cdist(class_features, centroid).squeeze(1)  # (n_class,)

        # Select closest nodes
        n = min(per_class, len(class_indices))
        _, top_idx = dists.topk(n, largest=False)
        selected.append(class_indices[top_idx].cpu())
...
def run_replay_experiment(buffer_size, data, class_weights, device, seed):
    """R1 train → build herding buffer → R2 train with replay."""
    ...
    # Build herding buffer on CPU (deterministic, no seed needed)
    replay_indices = build_herding_buffer(data, buffer_size)
    label_counts = {LABEL_NAMES[l]: (data.y[replay_indices] == l).sum().item()
                    for l in PYTHON_LABELS}
    print(f"        Buffer: {len(replay_indices)} nodes, composition: {label_counts}")
...
def _plot_training_curves(all_results, summary):
    ...
    plt.suptitle(f"Herding Replay (buffer={best}) — Training Dynamics",
...
def _plot_accuracy_comparison(all_results, summary):
    ...
    ax.set_title(f"Herding Replay (buffer={best}) — Per-Subreddit Accuracy\n"
...
def _plot_forgetting_viz(all_results, summary):
    ...
    ax.set_title(f"Herding Replay (buffer={best}) — Forgetting",
...
def _plot_buffer_sweep(summary):
    ...
    ax1.set_title("Herding Replay — Buffer Size Sweep", fontsize=14, fontweight="bold")
...
def main():
    ...
    print(f"Method: Herding Experience Replay")
