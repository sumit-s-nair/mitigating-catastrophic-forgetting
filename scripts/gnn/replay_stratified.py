"""
Experience Replay — Stratified Buffer Selection
================================================
Equal N/3 nodes sampled per R1 class (python=0, learnpython=1, django=2).
Ensures balanced class representation in the replay buffer.

Sweep buffer sizes: [100, 500, 1000, 2000]
Saves to ./res/replay_stratified/
"""
...
RES_DIR = PROJECT_ROOT / "res" / "replay_stratified"
RES_DIR.mkdir(parents=True, exist_ok=True)


# ─── Buffer Construction ─────────────────────────────────────────────────────

def build_stratified_buffer(data, buffer_size, seed):
    """Select buffer_size/3 nodes per R1 class for balanced representation.
    ...
    for label_id in PYTHON_LABELS:
        ...
        class_indices = class_mask.nonzero(as_tuple=True)[0].cpu()
        perm = torch.randperm(len(class_indices), generator=gen)
        n = min(per_class, len(class_indices))
        selected.append(class_indices[perm[:n]])
...
def run_replay_experiment(buffer_size, data, class_weights, device, seed):
    """R1 train → build stratified buffer → R2 train with replay."""
    ...
    # Build stratified replay buffer on CPU
    replay_indices = build_stratified_buffer(data, buffer_size, seed)
    # Report per-class composition
...
def _plot_training_curves(all_results, summary):
    ...
    plt.suptitle(f"Stratified Replay (buffer={best}) — Training Dynamics",
...
def _plot_accuracy_comparison(all_results, summary):
    ...
    ax.set_title(f"Stratified Replay (buffer={best}) — Per-Subreddit Accuracy\n"
...
def _plot_forgetting_viz(all_results, summary):
    ...
    ax.set_title(f"Stratified Replay (buffer={best}) — Forgetting",
...
def _plot_buffer_sweep(summary):
    ...
    ax1.set_title("Stratified Replay — Buffer Size Sweep", fontsize=14, fontweight="bold")
...
def main():
    ...
    print(f"Method: Stratified Experience Replay")
