"""
Topology-Aware Replay — High-Degree Node Selection
====================================================
Select replay buffer nodes by degree centrality: nodes with the most
connections are structurally important and should be prioritized for replay.

This is a graph-specific selection strategy (novel contribution) that
leverages the social graph structure to build better replay buffers.

Sweep buffer sizes: [100, 500, 1000, 2000]
Saves to ./res/tar_high_degree/
"""
...
RES_DIR = PROJECT_ROOT / "res" / "tar_high_degree"
...
def compute_node_degrees(data):
    """Compute degree of each node from edge_index (bidirectional)."""
    edge_index_cpu = data.edge_index.cpu()
    num_nodes = data.x.size(0)
    degrees = torch.zeros(num_nodes, dtype=torch.long)
    # Count occurrences in source side (already bidirectional)
    src = edge_index_cpu[0]
    degrees.scatter_add_(0, src, torch.ones_like(src))
    return degrees


def build_high_degree_buffer(data, buffer_size):
    """Select top-N R1 training nodes by degree centrality.
    Returns CPU tensor."""
    degrees = compute_node_degrees(data)

    # Get R1 training node indices
    ...
    _, top_idx = r1_degrees.topk(n, largest=True)
...
def run_tar_experiment(buffer_size, data, class_weights, device, seed):
    """R1 train → build high-degree buffer → R2 train with replay."""
    ...
    # Build high-degree buffer on CPU
    replay_indices = build_high_degree_buffer(data, buffer_size)
    degrees = compute_node_degrees(data)
    buf_degrees = degrees[replay_indices]
    print(f"        Buffer: {len(replay_indices)} nodes, "
          f"degree range: [{buf_degrees.min().item()}-{buf_degrees.max().item()}], "
          f"mean: {buf_degrees.float().mean().item():.1f}")
...
def _plot_training_curves(all_results, summary):
    ...
    plt.suptitle(f"TAR High-Degree (buffer={best}) — Training Dynamics",
...
def _plot_accuracy_comparison(all_results, summary):
    ...
    ax.set_title(f"TAR High-Degree (buffer={best})\nForgetting = {fgt['mean']:.4f} ± {fgt['std']:.4f}",
...
def _plot_forgetting_viz(all_results, summary):
    ...
    ax.set_title(f"TAR High-Degree (buffer={best}) — Forgetting", fontsize=14, fontweight="bold")
...
def _plot_buffer_sweep(summary):
    ...
    ax1.set_title("TAR High-Degree — Buffer Size Sweep", fontsize=14, fontweight="bold")
...
def _save_text_summary(summary):
    lines = ["=" * 65, "TOPOLOGY-AWARE REPLAY — HIGH-DEGREE NODE SELECTION", "=" * 65, ""]
...
def main():
    ...
    print(f"Method: Topology-Aware Replay (High-Degree)")
